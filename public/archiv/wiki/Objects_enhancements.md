...
Many, if not most, modern object oriented languages have support for a concept called accessor methods  [object methods](https://web.archive.org/web/20130614002751/http://en.wikipedia.org/wiki/Method_%28computer_science%29). Basically, these are simple methods which control access to some or all object internals via some form of indirection.

Most of the languages which support them do not have direct language syntax support for accessors, including the Java code included in the link above, and REBOL. Instead, these languages just use methods with naming conventions that mark them as accessors to the reader, if not the compiler. See [JavaBeans](https://web.archive.org/web/20130614002751/http://en.wikipedia.org/wiki/JavaBeans) for such an example, or the SET-\* and GET-\* accessors in REBOL 3's GUI.
